...
#define MADEV_EINT_PIN     	(GPIO5| 0x80000000)
#define MADEV_EN_PIN         (GPIO9| 0x80000000) 
struct cdev sdev;
#endif
...
static int mas_open(struct inode *inode, struct file *filp) {
	...
	mutex_lock(&dev_lock);
#if  0//def MTK
	if (hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_3000, MODE_NAME)!=1) { // mtk
		printw("%s: Fail to enable analog power\n", __func__);
		return -1;
	}
#endif
