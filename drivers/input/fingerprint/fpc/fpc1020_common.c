...
static const char *chip_text[] = {
	...
	"fpc1021f", 	/* FPC1020_CHIP_1021F *///add new chip id by jack.zheng 20151102
};
...
static const struct chip_struct chip_data[] = {
	...
	{FPC1020_CHIP_1021F, 0x021f, 1, FPC1021_ROWS, FPC1021_COLUMNS, FPC102X_ADC_GROUP_SIZE, 8000},//add new chip id by jack.zheng 20151102
...
int fpc1020_manage_huge_buffer(fpc1020_data_t *fpc1020, size_t new_size)
{
	...
	if (error) {
		...
		dev_dbg(&fpc1020->spi->dev, "%s, size=%d bytes\n",
...
int fpc1020_setup_defaults(fpc1020_data_t *fpc1020)
{
    ...
    case FPC1020_CHIP_1021F://add new chip id by jack.zheng 20151102
...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		usleep(FPC1020_RESET_HIGH1_US);

		gpio_set_value(fpc1020->reset_gpio, 0);
		usleep(FPC1020_RESET_LOW_US);

		gpio_set_value(fpc1020->reset_gpio, 1);
		usleep(FPC1020_RESET_HIGH2_US);
		...
		if (!error) {
			...
			dev_dbg(&fpc1020->spi->dev, "%s timed out,retrying ...\n",
				__func__);
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	dev_err(&fpc1020->spi->dev,"hardware_id=0x%x, match=%d", hardware_id, match);
...
static int fpc1020_write_sensor_1021_setup(fpc1020_data_t *fpc1020)
{
	...
	temp_u16 = 10;//0x0019;//25*0.006=150ms, modify by jack.zheng for reduce intt polling time
	FPC1020_MK_REG_WRITE(reg, FPC1020_REG_FNGR_DET_CNTR, &temp_u16);
	error = fpc1020_reg_access(fpc1020, &reg);
	if (error)
		goto out;
...
static int fpc1020_write_sensor_1025_setup(fpc1020_data_t *fpc1020)
{
	...
	temp_u16 = 0x00ff;	/* chang 0xff -> 0xf by billy */
	FPC1020_MK_REG_WRITE(reg, FPC1020_REG_FNGR_DET_CNTR, &temp_u16);
	error = fpc1020_reg_access(fpc1020, &reg);
	if (error)
		goto out;
...
int fpc1020_wait_finger_present(fpc1020_data_t *fpc1020)
{
	...
	while (1) {
		...
		if (error >= 0) {
			error = fpc1020_read_irq(fpc1020, true);
			if (error < 0){
				dev_err(&fpc1020->spi->dev, "%s IRQ not LOW after clear. error=%d\n", __func__, error);
#if 1 
				return error;
#else
				/* fix irq not clear err */
				error = fpc1020_read_status_reg(fpc1020);				
				if (error != FPC1020_STATUS_REG_RESET_VALUE)	{
					dev_err(&fpc1020->spi->dev," status check fail 0x%x!=0x1e .\n", error);
				}
			...
			if (error &
				...
				error = -ERANGE;//error = -EIO;//modify by jack.zheng for recognize error irq
			}
			return error;
		}
		...
		if (error < 0) {
			if (fpc1020->worker.stop_request){
				dev_err(&fpc1020->spi->dev, "worker stop request!!!\n");
				return -EINTR;
			}
			...
			//if (error == -ETIMEDOUT)
			//	dev_dbg(&fpc1020->spi->dev, "no finger down time out !!!\n");
		}
	}
}
...
int fpc1020_sleep(fpc1020_data_t *fpc1020, bool deep_sleep)
{
	...
	while (!sleep_ok && retries && (error >= 0)) {
		...
		if (!sleep_ok) {
			...
			usleep(FPC1020_SLEEP_RETRY_TIME_US);
			retries--;
		}
	}
...
EXPORT_SYMBOL_GPL(fpc1020_sleep);
					...
					int measured_val)
{
	...
	new_val = measured_val + 130; // Todo: awaiting calculated values//jack.zheng
