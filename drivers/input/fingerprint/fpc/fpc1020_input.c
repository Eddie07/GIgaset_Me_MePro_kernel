...
/* driver constants							*/
/* -------------------------------------------------------------------- */
#define FPC1020_KEY_FINGER_PRESENT	KEY_F18 /* KEY_WAKEUP*/
...
int fpc1020_input_task(fpc1020_data_t *fpc1020)
{
	...
	while (!fpc1020->worker.stop_request && !error) {
        ...
        getnstimeofday(&fpc1020->t_now);
        fpc1020->t_delta = timespec_sub(fpc1020->t_now, fpc1020->t_last);
        
		if (error == 0 && fpc1020->t_delta.tv_sec>=2) {
...
#if 1//software reset by jack.zheng for hanlde Unexpected IRQ
        if(error==-ERANGE){
            ...
            if (reset == 0 && status >= 0 &&
                (fpc1020_status_reg_t)(status & status_mask) == FPC1020_STATUS_REG_IN_IDLE_MODE) {
				error=0;
                dev_err(&fpc1020->spi->dev, "fpdebug ERROR IRQ, reset OK\n");
            } else {
                dev_err(&fpc1020->spi->dev, "fpdebug ERROR IRQ, reset FAILED\n");
            }
        }
#endif
	}
...
static int fpc1020_wait_finger_present_lpm(fpc1020_data_t *fpc1020)
{
	...
	const int zmask_ext = 0x0066U;//FPC1020_FINGER_DETECT_ZONE_MASK;//modify by jack.zheng for avoid wakeup slow. Area 5/6/9/10
