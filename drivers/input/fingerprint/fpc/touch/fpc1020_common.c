...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		if (!error) {
			...
			dev_warn(&fpc1020->spi->dev,  "%s timed out,retrying ...\n",
				__func__);
...
int fpc1020_read_irq(fpc1020_data_t *fpc1020, bool clear_irq)
{
	...
	if(error < 0){
		dev_err(&fpc1020->spi->dev,
			"%s: %s failed\n"
			, __func__, clear_irq ? "READ_INTERRUPT_WITH_CLEAR":"READ_INTERRUPT");
	...
	while (1) {
		...
		if (error >= 0) {
			...
			if (error &
				...
				dev_info(&fpc1020->spi->dev, "Finger down\n");
