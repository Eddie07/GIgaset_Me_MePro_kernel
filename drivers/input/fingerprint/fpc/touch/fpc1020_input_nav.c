...
#define FPC1020_INPUT_POLL_INTERVAL	(35000u)
#define FPC1020_CLICK_INTERVAL		(11000u)
#define THRESHOLD_DURATION_TAP		1500/*long press threshold*/
#define THRESHOLD_DURATION_DTAP		1300
...
static void dispatch_dpad_event(fpc1020_data_t* fpc1020, int x, int y, int finger_status)
{
    ...
    switch (finger_status)
    {
        case FNGR_ST_DETECTED:
		        ...
		        input_report_key(fpc1020->touch_pad_dev, KEY_F18 /*KEY_POWER*/, 1);
		        input_sync(fpc1020->touch_pad_dev);
		        input_report_key(fpc1020->touch_pad_dev, KEY_F18 /*KEY_POWER*/, 0);
		        input_sync(fpc1020->touch_pad_dev);
						msleep(500);
						#ifdef FPC1020_NAV_DPAD_DBG
		        dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report finger present\n");
						#endif
        ...
        case FNGR_ST_LOST:
            ...
            input_sync(fpc1020->touch_pad_dev);
						...
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report finger up\n");
						#endif
        ...
        case  FNGR_ST_TAP:
						#if 0 /* close by billy */
            input_report_key(fpc1020->touch_pad_dev, KEY_HOME, 1);
            input_sync(fpc1020->touch_pad_dev);						
            input_report_key(fpc1020->touch_pad_dev, KEY_HOME, 0);
            input_sync(fpc1020->touch_pad_dev);
						#endif
						
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report single click\n");
						#endif
        ...
        case FNGR_ST_DOUBLE_TAP:
            ...
            //msleep(500);
						...
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report double click\n");
						#endif
        ...
        case FNGR_ST_HOLD:
            ...
            fpc1020->nav.tap_status = -1;
						...
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report long press\n");
						#endif
        ...
        case FNGR_ST_MOVING:
						...
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] %s: raw axis x=%d, y=%d\n", __func__, x, y);
						#endif
            ...
            y = y * fpc1020->nav.p_multiplier_y / FLOAT_MAX;
						...
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_info(&fpc1020->spi->dev, "[FPC/DPAD] %s:multiplied axis x=%d, y=%d , abs_x=%d, abs_y=%d\n", 
            ...
            if (reset)
            {
                ...
                issued_key = 0;
						...
						#ifdef FPC1020_NAV_DPAD_DBG
            dev_info(&fpc1020->spi->dev, "[FPC/DPAD] Navigation:active=%d, throw_event=%d, fpc1020->nav.sum_x:%d, fpc1020->nav.sum_y:%d, nav.threshold_key_start:%d, nav.threshold_key_offset=%d\n",
                    active, fpc1020->nav.throw_event, fpc1020->nav.sum_x, fpc1020->nav.sum_y, fpc1020->nav.threshold_key_start, fpc1020->nav.threshold_key_offset);
						#endif
            ...
            if(fpc1020->nav.throw_event == 0)
            {
                if (abs_x > abs_y && fpc1020->nav.sum_x > fpc1020->nav.threshold_key_start
                    && (fpc1020->nav.sum_y < fpc1020->nav.threshold_key_offset
                        && fpc1020->nav.sum_y > -fpc1020->nav.threshold_key_offset))
                {
                    key_code = KEY_DOWN; /*chang  KEY_RIGHT -> KEY_DOWN  by billy */
                    ...
                    fpc1020->nav.sum_y = 0;
                         ...
                         && (fpc1020->nav.sum_y < fpc1020->nav.threshold_key_offset
                             && fpc1020->nav.sum_y > -fpc1020->nav.threshold_key_offset))
                {
                    key_code = KEY_UP;	/*chang  KEY_LEFT -> KEY_UP  by billy */
                    fpc1020->nav.sum_x += fpc1020->nav.threshold_key_start;
                    fpc1020->nav.sum_y = 0;
                }
                ...
                else if (abs_x < abs_y && fpc1020->nav.sum_y < -fpc1020->nav.threshold_key_start)
                {
                    ...
                    key_code = KEY_RIGHT; /*chang  KEY_UP -> KEY_RIGHT  by billy */
                ...
                else if (abs_x < abs_y && fpc1020->nav.sum_y > fpc1020->nav.threshold_key_start)
                {
                    ...
                    key_code = KEY_LEFT;	/*chang  KEY_DOWN -> KEY_LEFT  by billy */
                    fpc1020->nav.sum_y -= fpc1020->nav.threshold_key_start;
                    fpc1020->nav.sum_x = 0;
                }
                else
                ...
                {
                    active = 1;
                    //if (fpc1020->nav.move_up_cnt > 0 || fpc1020->nav.move_down_cnt > 1) /* close by billy */
                    { 
                    		#if 0 /* close by billy */
                        input_report_key(fpc1020->touch_pad_dev, key_code, 1);
                        input_sync(fpc1020->touch_pad_dev);
                        ...
                        {
                            case KEY_UP:
                                dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report move up\n");
                                break;
                            case KEY_DOWN:
                                dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report move down\n");
                                break;
                            case KEY_LEFT:
                                dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report move left\n");
                                break;
                            case KEY_RIGHT:
                                dev_warn(&fpc1020->spi->dev, "[FPC/DPAD] report move right\n");
                                break;
                            default:
                                break;
                        }
                    ...
                    issued_key = key_code;
										//fpc1020->nav.report_keys = key_code; /* add by billy */
                }
                else
                {
                    return;
...
static void dispatch_touch_event(fpc1020_data_t *fpc1020, int x, int y, int finger_status)
{
	...
	switch(finger_status) {
		...
		case FNGR_ST_DOUBLE_TAP:
			...
			dev_info(&fpc1020->spi->dev, "[FPC/TOUCH] report double click\n");
			#endif
			break;
		...
		case FNGR_ST_HOLD:
			...
			mdelay(50);
			#ifdef FPC1020_NAV_TOUCH_DBG
			dev_info(&fpc1020->spi->dev, "[FPC/TOUCH] report long press\n");
			#endif
			break;
		...
		case FNGR_ST_MOVING:
			...
			dev_info(&fpc1020->spi->dev, "[WYQ] raw x = %d, y = %d\n", x, y);
      ...
      dev_info(&fpc1020->spi->dev, "[WYQ] after x=%d, y=%d, sumx=%d, sumy=%d\n", x, y, fpc1020->nav.nav_sum_x, fpc1020->nav.nav_sum_y);
...
static void process_navi_event(fpc1020_data_t* fpc1020, int dx, int dy, int finger_status)
{
    ...
    if (tick_down > 0)
    {
        ...
        dev_info(&fpc1020->spi->dev, "%s:deviation=%d, duration=%ld, finger_status= %s\n",
                __func__, deviation, duration, 
                (finger_status==1) ? "DETECTED":
                (finger_status==2) ? "LOST":
								(finger_status==3) ? "TAP":
								(finger_status==4) ? "HOLD":
								(finger_status==5) ? "MOVING":
								(finger_status==6) ? "L_HOLD":
								(finger_status==7) ? "DOUBLE_TAP":"NONE");
				#endif
        ...
        if (deviation > THRESHOLD_RANGE_TAP)
        {
            ...
            fpc1020->nav.tap_status = -1;
						...
						#ifdef FPC1020_NAV_DBG
            dev_info(&fpc1020->spi->dev, "%s:throw the events\n", __func__);
						#endif
						
            if (duration > THRESHOLD_DURATION_TAP)
            {
								#ifdef FPC1020_NAV_DBG
                dev_info(&fpc1020->spi->dev, "%s: prepare long press because of outside\n", __func__);
								#endif
        ...
        else
        {
            if (duration < THRESHOLD_DURATION_TAP)
            {
                if (finger_status == FNGR_ST_LOST && fpc1020->nav.detect_zones > 0) /* test 2-> 0 by billy */
                {
											...
											&& ((deviation == 0)||((dy == fpc1020->nav.move_pre_y)&& (dx==fpc1020->nav.move_pre_x))))
                    {
                        ...
                        //fpc1020->nav.detect_zones = 0;
												#ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "%s:prepare report double click\n", __func__);
												#endif
                    }
                    else if (deviation <= THRESHOLD_RANGE_MIN_TAP || ((dy == fpc1020->nav.move_pre_y)&& (dx==fpc1020->nav.move_pre_x)))
                    {
                        ...
                        fpc1020->nav.tap_start = tick_curr;
												#ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "%s:prepare report single click\n", __func__);
												#endif
                    }
										/* add by billy */
                    else if ((deviation <= THRESHOLD_RANGE_MIN_TAP ||
                   		((dy == fpc1020->nav.move_pre_y)&& (dx==fpc1020->nav.move_pre_x)))
                				&& tick_curr - fpc1020->nav.tap_start > THRESHOLD_DURATION_TAP)
		   							{
												#ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, " %s: prepare report long1 press\n", __func__);
												#endif
                    ...
                    {
												#ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "%s: still report finger lost\n", __func__);
												#endif
                   ...
                   ((dy == fpc1020->nav.move_pre_y)&& (dx==fpc1020->nav.move_pre_x)))
										&& tick_curr - fpc1020->nav.tap_start > THRESHOLD_DURATION_TAP /* test by billy && fpc1020->nav.throw_event != 1 */)
            {
								#ifdef FPC1020_NAV_DBG
                dev_info(&fpc1020->spi->dev, "%s: prepare report long press\n", __func__);
								#endif
    ...
    dev_dbg(&fpc1020->spi->dev, "[ %s : %s ] dx : %d / dy : %d\n", 
			(fpc1020->nav.input_mode==0)? "TRACKPAD":		
			(fpc1020->nav.input_mode==1)? "MOUSE":	
			(fpc1020->nav.input_mode==2)? "TOUCH":"DPAD",
			(finger_status==1) ? "DETECTED":
			(finger_status==2) ? "LOST":
			(finger_status==3) ? "TAP":
			(finger_status==4) ? "HOLD":
    ...
    switch (fpc1020->nav.input_mode)
    {
        ...
        case FPC1020_INPUTMODE_TOUCH:
            ...
            break;
        default:
            dev_err(&fpc1020->spi->dev, "undefined input mode\n");
            break;
    }
}
...
int  fpc1020_input_init(fpc1020_data_t* fpc1020)
{
    ...
    if ((fpc1020->chip.type != FPC1020_CHIP_1020A)
        ...
        && (fpc1020->chip.type != FPC1020_CHIP_1150A)
				&& (fpc1020->chip.type != FPC1020_CHIP_1025X)) /* add by billy */
    {
		...
		else
    {
        ...
        input_set_capability(fpc1020->input_dev, EV_KEY, KEY_F18); 
		...
		else
    {
        ...
        set_bit(KEY_HOME, fpc1020->touch_pad_dev->keybit);
        set_bit(KEY_POWER, fpc1020->touch_pad_dev->keybit);
        input_set_capability(fpc1020->touch_pad_dev, EV_KEY, KEY_ENTER);
        input_set_capability(fpc1020->touch_pad_dev, EV_KEY, KEY_BACK);
        input_set_capability(fpc1020->touch_pad_dev, EV_KEY, KEY_HOME);
        input_set_capability(fpc1020->touch_pad_dev, EV_KEY, KEY_POWER);			
        input_set_capability(fpc1020->touch_pad_dev, EV_KEY, KEY_F18); 			
    ...
    dev_info(&fpc1020->spi->dev, "%s %d %d %d %d\n", __func__, 
...
int fpc1020_input_task(fpc1020_data_t* fpc1020)
{
           ...
           fpc1020->nav.enabled && (error >= 0))
    {
        ...
        dev_dbg(&fpc1020->spi->dev, "report touch zones=%d\n", fpc1020->nav.detect_zones);
        ...
        while (!fpc1020->worker.stop_request && (error >= 0))
        {
            ...
            {
								#ifdef  FPC1020_NAV_DBG
                dev_info(&fpc1020->spi->dev, "prepare report finger up #1\n");
								#endif
                ...
                if (click_diffTime <= FPC1020_CLICK_INTERVAL && fpc1020->nav.detect_zones == 0)
                {
                    ...
                    {
												#ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "prepare fast report#1 double click\n");
												#endif
                    ...
                    {
                        #ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "[FPC] prepare fast report#1 single click\n");
                        #endif
                        fpc1020->nav.tap_status = FNGR_ST_TAP;
            ...
            {
								#ifdef	FPC1020_NAV_DBG
								dev_info(&fpc1020->spi->dev, "prepare report finger up #2\n");
								#endif
                ...
                if (click_diffTime <= FPC1020_CLICK_INTERVAL && fpc1020->nav.detect_zones == 0)
                {
                    ...
                    {
                        #ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "prepare fast report#2 double click\n");
                        #endif
                        process_navi_event(fpc1020, 0, 0, FNGR_ST_DOUBLE_TAP);
                    }
                    else
                    {
                        ...
                        fpc1020->nav.tap_start = jiffies * 1000 / HZ;
                        #ifdef FPC1020_NAV_DBG
                        dev_info(&fpc1020->spi->dev, "prepare fast report#2 single click\n");
                        #endif
                    }
                }
            ...
            sumY += dy;
						...
						#ifdef FPC1020_NAV_DBG
            dev_info(&fpc1020->spi->dev, "get_movement dx=%d, dy=%d, sumx=%d, sumy=%d\n", dx, dy, sumX, sumY);
						#endif
            ...
            if (diffTime > 0)
            {
                ...
                {
										#ifdef FPC1020_NAV_DBG
                    dev_info(&fpc1020->spi->dev, "move poll \n");
										#endif
...
static int fpc1020_write_nav_setup(fpc1020_data_t* fpc1020)
{
    //const int mux = 2; //close by billy fix build err " unused variable 'mux' [-Wunused-variable] " */
    int error = 0;
    u16 temp_u16;
    u32 temp_u32;
    u8 temp_u8;
...
static int capture_nav_image(fpc1020_data_t* fpc1020)
{
    ...
    dev_dbg(&fpc1020->spi->dev, "capture_nav_image report grey count =%d\n", fpc1020_check_image(fpc1020,NAV_IMG_W,NAV_IMG_H));
