...
#include "fpc1020_navlib.h" /* add by billy */
MODULE_LICENSE("GPL");
...
#ifdef CONFIG_ARCH_MSM /* add by billy */
...
#ifdef CONFIG_INPUT_FPC1020_NAV /* add by billy */
...
static FPC1020_DEV_ATTR(diag, nav_en, DEVFS_DIAG_MODE_RW); /* add by billy */
...
static struct attribute *fpc1020_diag_attrs[] = {
    ...
    &fpc1020_attr_nav_en.attr.attr, /* add by billy */
	NULL
};
...
/* add by billy */
	...
	fpc1020->spi = spi;
	fpc1020->spi_freq_khz = 4800u; /* change 1000->4800 */
...
#ifdef CONFIG_INPUT_FPC1020_NAV /* change by billy */
...
#if 0 /* change by billy */
	fpc1020_worker_goto_idle(fpc1020);
	//fpc1020_wake_up(fpc1020); /* add by billy */
					...
					struct fpc1020_platform_data *pdata)
{
	...
	pdata->reset_gpio = of_get_named_gpio_flags(node,
			"fpc,gpio_reset", 0, &pdata->reset_flags);
	pdata->irq_gpio = of_get_named_gpio_flags(node,
			"fpc,gpio_irq", 0, &pdata->irq_flags);
	pdata->cs_gpio = of_get_named_gpio_flags(node,
			"fpc,gpio_cs", 0, &pdata->cs_flags);

	pdata->use_regulator_for_bezel = 1;
	pdata->use_fpc2050 = 1;
#endif
...
#ifdef DEBUG_INPUT_MODE /* add by billy */
					...
					char *buf)
{
	...
	case offsetof(fpc1020_diag_t, spi_regsize):
		...
		break;
    case offsetof(fpc1020_diag_t, nav_en): /* add by billy */
    	val = (int)fpc1020->diag.nav_en;
        break;
					...
					size_t count)
{
	...
	}else if(fpc_attr->offset == offsetof(fpc1020_diag_t, nav_en)){ /* add by billy */
        error = kstrtou64(buf, 0, &val);
        if (!error)
        {
            if(val == 0)
            {
                if(fpc1020->diag.nav_en == 1)
                {
                    fpc1020->diag.nav_en = val;
                    fpc1020_start_input(fpc1020);
                }
            }
            else if(val == 1)
            ...
            printk("[FPC]navigation enable = %d \n",fpc1020->diag.nav_en);
        }
...
static int fpc1020_new_job(fpc1020_data_t *fpc1020, int new_job)
{
	dev_dbg(&fpc1020->spi->dev, "%s %s\n", __func__, 
		(new_job==0) ? "IDLE MODE":
		(new_job==1) ? "CAPTURE MODE":
		(new_job==2) ? "INPUT MODE":"EXIT");
