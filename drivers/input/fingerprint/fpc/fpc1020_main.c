...
static const struct dev_pm_ops fpc1020_pm = {
	...
	SET_SYSTEM_SLEEP_PM_OPS(fpc1020_suspend, fpc1020_resume)
...
static FPC1020_DEV_ATTR(diag, unlock_setting,		DEVFS_DIAG_MODE_RW);
static FPC1020_DEV_ATTR(diag, input_enable,		DEVFS_DIAG_MODE_RW);
...
static struct attribute *fpc1020_diag_attrs[] = {
	...
	&fpc1020_attr_unlock_setting.attr.attr,		
	&fpc1020_attr_input_enable.attr.attr,	
		...
		unsigned long event, void *data)
{
	...
	dev_dbg(&fpc1020->spi->dev, "%s event 0x%lx\n", __func__, event);
	
	if (evdata && evdata->data && fpc1020 && fpc1020->spi) {
		if (event == FB_EVENT_BLANK) {
			transition = evdata->data;
			if (*transition == FB_BLANK_POWERDOWN) {
				//fpc1020_suspend(&fpc1020->spi->dev);
				fpc1020->fb_state = false;
				dev_err(&fpc1020->spi->dev, "%s off\n", __func__);
			} else if (*transition == FB_BLANK_UNBLANK) {
				//fpc1020_resume(&fpc1020->spi->dev);
				dev_err(&fpc1020->spi->dev, "%s on\n", __func__);
				fpc1020->fb_state = true;
				fpc1020->sleep_status=false;
...
static void fpc1020_configure_sleep(fpc1020_data_t *fpc1020)
{
	int retval = 0;

	fpc1020->fb_notif.notifier_call = fpc1020_fb_notifier_cb;
...
static int fpc1020_event_pinctrl_configure(fpc1020_data_t *fpc1020, bool active)
{
	...
	if (active) {
		set_state = pinctrl_lookup_state(fpc1020->pinctrl, "finger_event_active");
		if (IS_ERR(set_state)) {
			dev_err(&fpc1020->spi->dev, "cannot get event pinctrl active state\n");
			return PTR_ERR(set_state);
		}
		
		fpc1020->gpio_state_active = set_state;
		
	} else {
		set_state = pinctrl_lookup_state(fpc1020->pinctrl, "finger_event_suspend");
		if (IS_ERR(set_state)) {
			dev_err(&fpc1020->spi->dev, "cannot get event pinctrl sleep state\n");
			return PTR_ERR(set_state);
		}
		
		fpc1020->gpio_state_suspend = set_state;
	}
	
	retval = pinctrl_select_state(fpc1020->pinctrl, set_state);
	if (retval) {
		dev_err(&fpc1020->spi->dev, "cannot set event pinctrl active state\n");
		return retval;
	}
...
static int __devinit fpc1020_probe(struct spi_device *spi)
{
	...
	fpc1020->spi = spi;
	fpc1020->spi_freq_khz = 4800u; /* 1000-> 4800 */
	...
	fpc1020->use_fpc2050 = 1; /* 0->1*/ 
	fpc1020->diag.unlock_setting =0;

	/* Get pinctrl if target uses pinctrl */
	fpc1020->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(fpc1020->pinctrl)) {
		if (PTR_ERR(fpc1020->pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		dev_warn(dev, "Target does not use pinctrl\n");
		fpc1020->pinctrl = NULL;
	}

#if 0
	if (fpc1020->pinctrl) {
		error = fpc1020_event_pinctrl_configure(fpc1020, false);
		if (error) {
			dev_err(dev, "cannot set event pinctrl active state\n");
			goto err;
		}
	}
#endif
	...
	fpc1020_configure_sleep(fpc1020);

	device_init_wakeup(&fpc1020->spi->dev, 1);
	wake_lock_init(&fpc1020->wakelock, WAKE_LOCK_SUSPEND, dev_name(&fpc1020->spi->dev));
...
static int fpc1020_suspend(struct device *dev)
{
	...
	if(fpc1020->diag.unlock_setting==1){
		if (fpc1020->pinctrl) {
			ret = fpc1020_event_pinctrl_configure(fpc1020, false);
			if (ret) {
		...
		if (ret) {
			dev_err(dev, "failed to set fpc1020 suspend state\n");
			return ret;
		}
		...
		if(fpc1020->pm_time < 300000){
			fpc1020->pm_sum++;
			if(fpc1020->pm_sum > PM_SUM+1)
				fpc1020->pm_sum=0;
		}else{
				fpc1020->pm_sum=0;
		}
	...
	fpc1020->suspended=true;
	fpc1020->sleep_status=true;

	getnstimeofday(&pm_t1);
...
static int fpc1020_resume(struct device *dev)
{
	...
	getnstimeofday(&pm_t2);
	ts_delta = timespec_sub(pm_t2, pm_t1);
	fpc1020->pm_time =
	...
	if(fpc1020->diag.unlock_setting==1){
		if (fpc1020->pinctrl) {
			ret = fpc1020_event_pinctrl_configure(fpc1020, true);
			if (ret) {
	...
	fpc1020->suspended= false;
	fpc1020->sleep_status=false;

	if (fpc1020->input.enabled){//if(fpc1020->pm_sum>PM_SUM){ //modify by jack.zheng for cannot wakeup
		fpc1020_start_input(fpc1020);
		wake_lock_timeout(&fpc1020->wakelock, msecs_to_jiffies(WAKE_HOLD_TIME));
	}
...

{
	...
	if (down_interruptible(&fpc1020->mutex)){
		dev_err(&fpc1020->spi->dev, "%s fail\n", __func__);
		return -ERESTARTSYS;
	}
...
#if 1
        fpc1020->capture.state = FPC1020_CAPTURE_STATE_COMPLETED;//reset state machine by jack.zheng 20160112
		fpc1020_start_input(fpc1020);
#endif


#if 0
	if (!fpc1020->input.enabled && fpc1020->sleep_status){
...
static unsigned int fpc1020_poll(struct file *file, poll_table *wait)
{
	...
	if (fpc1020->capture.available_bytes > 0)
		...
		blocking_op =
			...
			(mode == FPC1020_MODE_WAIT_FINGER_DOWN)) ? true : false;
#else
		blocking_op =
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->reset_gpio)) {
		...
		dev_dbg(&fpc1020->spi->dev, "Using soft reset\n");
...
#if 1	
		/* reset, irq gpio info */
		pdata->reset_gpio = of_get_named_gpio_flags(node, "fpc,gpio_reset", 0, &pdata->reset_flags);
		pdata->irq_gpio 	= of_get_named_gpio_flags(node, "fpc,gpio_irq", 0, &pdata->irq_flags);
		pdata->cs_gpio 		= of_get_named_gpio_flags(node, "fpc,gpio_cs", 0, &pdata->cs_flags);
#endif
...
irqreturn_t fpc1020_interrupt(int irq, void *_fpc1020)
{
	...
	if (gpio_get_value(fpc1020->irq_gpio)) {
		if(fpc1020->sleep_status){
			//dev_err(&fpc1020->spi->dev, "%s wake up!\n", __func__);
			fpc1020->suspended=false;
			...
			//return IRQ_HANDLED;
			wake_lock_timeout(&fpc1020->wakelock, msecs_to_jiffies(WAKE_HOLD_TIME));
            getnstimeofday(&fpc1020->t_last);
            dev_err(&fpc1020->spi->dev, "fpdebug event input_sync **************\n");
		}
					...
					char *buf)
{
	...
	case offsetof(fpc1020_diag_t, selftest):
		...
		break;
	case offsetof(fpc1020_diag_t, unlock_setting):
		val = fpc1020->diag.unlock_setting;
		break;
	case offsetof(fpc1020_diag_t, input_enable):
		val = fpc1020->diag.input_enable;
		break;
					...
					size_t count)
{
	...
	} else if (fpc_attr->offset == offsetof(fpc1020_diag_t, unlock_setting)) {
			error = kstrtou64(buf, 0, &val);

			if (!error)
				fpc1020->diag.unlock_setting=(u8)val;
	}else if (fpc_attr->offset == offsetof(fpc1020_diag_t, input_enable)) {
			error = kstrtou64(buf, 0, &val);

			if (!error)
				fpc1020->diag.input_enable=(u8)val;
...
EXPORT_SYMBOL_GPL(fpc1020_worker_goto_idle);
...
{
	dev_dbg(&fpc1020->spi->dev, "%s [ %s ]\n", __func__, 
		(new_job==FPC1020_WORKER_IDLE_MODE) ?"IDLE MODE":	
		(new_job==FPC1020_WORKER_CAPTURE_MODE) ?"CAPTURE MODE":	
		(new_job==FPC1020_WORKER_INPUT_MODE) ?"INPUT MODE":"EXIT");
...
EXPORT_SYMBOL_GPL(fpc1020_start_input);
