...
#define FW_IMAGE_NAME_421 "synaptics/startup_fw_update_421.img"
#define FW_IMAGE_NAME_427 "synaptics/startup_fw_update_427.img"
...
//pr_err("[lwj]%s:--5\n",__func__);
exit:
rmi4_data->device_id=device_config_id;
rmi4_data->image_id=image_config_id;
...
static int fwu_start_reflash(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
//dev_err(rmi4_data->pdev->dev.parent,"%s---1\n",__func__);
	if (rmi4_data->sensor_sleep) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Sensor sleeping\n",
				__func__);
...
//dev_err(rmi4_data->pdev->dev.parent,"%s---2\n",__func__);
	rmi4_data->stay_awake = true;
	...
	if (fwu->image == NULL) {
		...
		{
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_427, sizeof(FW_IMAGE_NAME_427),
				sizeof(FW_IMAGE_NAME_427));
		} //17421
		else if((device_version>=DEVICE_VERSION_17421)&&(device_version<=DEVICE_VERSION_17421_MP))
		{
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_421, sizeof(FW_IMAGE_NAME_421),
				sizeof(FW_IMAGE_NAME_421));	  
		} //SLOANL
		else if((device_version>=DEVICE_VERSION_SLOANL)&&(device_version<=DEVICE_VERSION_SLOANL_MP))
		{
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_421, sizeof(FW_IMAGE_NAME_421),
				sizeof(FW_IMAGE_NAME_421));
		...
		dev_dbg(rmi4_data->pdev->dev.parent,
				...
				__func__, fwu->image_name);
dev_err(rmi4_data->pdev->dev.parent,"%s---go ...\n",__func__);
...
dev_err(rmi4_data->pdev->dev.parent,"%s: Firmware image: %s \n",
					__func__, fwu->image_name);
...
exit:
	...
	rmi4_data->stay_awake = false;
//dev_err(rmi4_data->pdev->dev.parent,"%s---e\n",__func__);
	return retval;
}
