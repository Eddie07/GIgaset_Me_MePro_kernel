...
static int escore_slim_probe(struct slim_device *sbdev)
{
	...
	if (sbdev->dev.of_node) {
		/* Add for ES704 startup */
		/*
		 * Change by Harry
		 * Slimbus and UART all need to startup
		 */
		if (escore->high_bw_intf != ES_INVAL_INTF) {
			//call startup sequence
			rc = escore_device_startup_seq(&sbdev->dev);
			if(!rc) {
				pr_err("%s, device startup successfully\n",
					__func__);
			} else {
				pr_err("%s, device startup failed\n",
					__func__);
			}
		}
