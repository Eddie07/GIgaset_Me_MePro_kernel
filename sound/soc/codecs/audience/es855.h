/*
 * es855.h  --  ES855 Soc Audio driver
 *
...
#include "escore.h"
/*
 * es-a350-reg.h are being developed in separated effort
 * these will be include when the headers are stable
*/
/*
#include "es-a350-reg.h"
*/
/* temporary to define of ES_MAX_REGISTER to enable successful build */
#define ES_MAX_REGISTER 1
...
extern struct snd_soc_codec_driver soc_codec_dev_es855;
extern struct snd_soc_dai_driver es855_dai[];

extern int es855_core_probe(struct device *dev);

extern int es855_bootup(struct escore_priv *es855);

#define es855_resp(obj) ((obj)->last_response)
int es855_cmd(struct escore_priv *es855, u32 cmd);
int es855_bus_init(struct escore_priv *es855);
int es855_boot_finish(struct escore_priv *es855);
int es855_set_streaming(struct escore_priv *escore, int value);
int es855_set_datalogging(struct escore_priv *escore, int value);
int es855_i2c_sleep(struct escore_priv *escore);
int es855_i2c_wakeup(struct escore_priv *escore);
int es855_slim_sleep(struct escore_priv *escore);
int es855_slim_wakeup(struct escore_priv *escore);
