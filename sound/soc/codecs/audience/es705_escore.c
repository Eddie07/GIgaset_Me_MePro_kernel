...
extern void tomtom_enable_micbaisc1(void);
...
// Change CONFIG_ARCH_MSM8974 to CONFIG_ARCH_MSM8994 by Harry
...
// Change 0666 to 0444 by Harry for CTS
static DEVICE_ATTR(pm_enable, 0444, es705_get_pm_enable, es705_set_pm_enable);
...
static int es705_fw_download(struct escore_priv *es705, int fw_type)
{
	...
	printk("es705->boot_ops.setup = %pF\n",es705->boot_ops.setup);
	if (rc) {
...
static int es705_put_internal_route(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	printk(KERN_ERR"es705_put_internal_route\n");
...
// Add by Harry
static const char * const es705_mclk_texts[] = {
	"OFF", "ON",
};
static const struct soc_enum es705_mclk_text_enum =
	SOC_ENUM_SINGLE(SND_SOC_NOPM, 0,
			ARRAY_SIZE(es705_mclk_texts),
			es705_mclk_texts);

int escore_mclk_enable_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	int rc = 0;
	
	pr_debug("%s:escore_priv.mclk_enable = %d\n",__func__,escore_priv.mclk_enable);
	
	if (escore_priv.mclk_enable == ucontrol->value.enumerated.item[0]) {
		pr_debug("%s: Not need change!\n",__func__);
		return 0;
	}

	escore_priv.mclk_enable = ucontrol->value.enumerated.item[0];

	if (escore_priv.mclk_enable) {
		rc  = clk_prepare(escore_priv.es_mclk);
		if (rc) {
			pr_err("%s:clk_prepare with error %d\n",__func__,rc);
			goto clk_exit;
		}
		rc  = clk_enable(escore_priv.es_mclk);
		if (rc) {
			pr_err("%s:clk_enable with error %d\n",__func__,rc);
			goto clk_exit;
		}
	} else {
		pr_debug("%s:Goto disbale mclk!!\n",__func__);
		clk_disable(escore_priv.es_mclk);
		clk_unprepare(escore_priv.es_mclk);
	}
...
int escore_mclk_enable_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = escore_priv.mclk_enable;
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
	...
	// Add by Harry to control MCLK
	SOC_ENUM_EXT("MCLK Enable", es705_mclk_text_enum,
			   escore_mclk_enable_get,
			   escore_mclk_enable_put),
   // End of add by Harry
};
...
static int es705_enable_mclk(int enable)
{
	int rc = 0;
	
	pr_debug("++es705_enable_mclk with %d\n",enable);
	
	if (escore_priv.mclk_enable == enable) {
		pr_debug("ES705 clock status no need change!\n");
		return 0;
	}

	if (! escore_priv.es_mclk) {
		pr_err("%s: mclk == NULL!!\n",__func__);
		return -EINVAL;
	}
	...
	escore_priv.mclk_enable = enable;
...
static struct esxxx_platform_data *es705_populate_dt_pdata(struct device *dev)
{
	...
	pdata->esxxx_clk_cb = es705_enable_mclk;
...
*/
	...
	/*
	 * Harry :
	 * Move this function up to fix crash
	 */
...
#endif
	// Add by Harry to focrce use SPI
//	escore_priv.pri_intf = ES_SPI_INTF;
