...
extern int tomtom_mic_bais1_enable(int enable);
...
escore_cvq_sleep_thread_exit:
    // Add by Harry to enable MICBISA1
	tomtom_mic_bais1_enable(1);
	return rc;
}
		...
		void *dev)
{
...
#endif
	u32 resp;
	u32 es_set_preset = (0x8031 << 16) | 0x1BBC; // 7100 
	int rc = 0;
	...
	if (voice_sense->cvs_preset != 0xFFFF &&
		...
		//////////////////////////////////////////////////////////////////////////
		// Note :
		// Add by Harry for Wakeup mode,if use CVQ mode,it will error
		// This command will send by APP in product use : tinymix "Preset" 7100
		rc = escore_cmd(escore,es_set_preset,&resp);
		/////////////////////////////////////////////////////////////////////////
		/* Each time earSmart chip comes in BOSKO mode after
		 * VS detect, CVS mode will be disabled */
		voice_sense->cvs_preset = 0;
	}
	pr_debug("%s:Goto send uevent dev path = %s!!\n",__func__,kobject_get_path(&escore->dev->kobj, GFP_KERNEL));
