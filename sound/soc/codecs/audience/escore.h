...
#define ENTER_FUNC()   printk(KERN_ERR"++%s:%d\n",__func__,__LINE__)
#define EXIT_FUN()     printk(KERN_ERR"--%s:%d\n",__func__,__LINE__)
...
/*
 * Add by Harry
 * Define the ES704 power min and max
 */
#define ES704_VDD_CORE_MIN 0
#define ES704_VDD_CORE_MAX 1500000
...
/* endif 705 */
	...
	int (*get_gpio_pinctrl)(struct device *dev);
	/* Add by Harry for VDD_CORE */
	int mclk_enable;
	int gpio_reset;
	int irq_lock;
	struct regulator *es_vdd_core;
	struct clk* es_mclk;
	// struct wake_lock es_wake_lock;
	/* End of add by Harry */
	int acc_det_status;
	int acc_btn_pressed;
};
