...
static char *filename_17421 = "smartpa/tfa98xx_17421.cnt";
static char *filename_17427 = "smartpa/tfa98xx_17427.cnt";

module_param(filename_17421, charp, 0);
module_param(filename_17427, charp, 0);
...
int tfa98xx_cnt_loadfile(struct tfa98xx *tfa98xx, int index)
{
	...
	pr_debug("I2C device: 0x%x\n", addr);
        ...
        if ((device_version == DEVICE_VERSION_17421)
                ...
                ret = request_firmware(&container, filename_17421, codec->dev);
                if (ret) {
                        pr_err("Failed to read %s", filename_17421);
                        return ret;
                }
        } else if((device_version == DEVICE_VERSION_17427)
                ...
                ret = request_firmware(&container, filename_17427, codec->dev);
                if (ret) {
                        pr_err("Failed to read %s", filename_17427);
                        return ret;
                }
        }
